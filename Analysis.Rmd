...
```
Here I created a new statistic that gives the goals scored per game, and analyzed a linear regression model between goals per game and the % of headers won. There is no significant correlation. It may also be that instead of scoring the goals from winning the aerial challenge, the striker may be providing an assist for a goal. 
...
Similarly here, we can see that there is no significant correlation between goals and assists per game with the players' heading ability. It seems that the Premier League clubs are being inefficient in the distribution of their funds when they are spending a premier to buy taller strikers, strictly in terms of the goals and assists they may provide. 
